...
class Textmining:
    ...
    def create_coocurence_score(self, top_n = 20, n_clusters = 3):  # Add n_clusters parameter for K-means
        if self.dtm is None:
            raise ValueError("error 001")
        ...
        # K-means clustering on the co-occurrence matrix
        kmeans = KMeans(n_clusters=n_clusters, random_state=42)
        kmeans.fit(filtered_matrix)
    ...
    # Create the graph
        ...
        # Use K-means cluster labels as node colors
        node_colors = [cluster_labels[i] for i in range(len(top_terms))]

        # Visualizing the graph
        plt.figure(figsize=(60, 80))
        pos = nx.spring_layout(G, k = 14,iterations=200,seed=42)
        ...
        plt.title("동시출현 네트워크 그래프 with K-Means Clustering", fontsize=20)
        plt.show()
